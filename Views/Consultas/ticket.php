...
for ($i=0; $i < count($data['productos']); $i++) { 
    $pdf->MultiCell(30,4,utf8_decode($data['productos'][$i]['producto']),0,'L'); 
    $pdf->Cell(35, -5, $data['productos'][$i]['cantidad'],0,0,'R');
    $pdf->Cell(10, -5, SMONEY.number_format(round($data['productos'][$i]['precio'],2), 2, ',', ' '),0,0,'R');
...
$pdf->Cell(20, 10, '', 0);
$pdf->Cell(15, 10, SMONEY.number_format(floatval($data['total']), 2, ',', ' '),0,0,'R');
