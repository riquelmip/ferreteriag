...
$pdf->Text(75, 15, utf8_decode('FERRETERIA GRANADEÑO'));
...
/////////////////////////////
//// Array de Cabecera
$header = array("Cod.", "Descripción","Cant.","Precio","Total");
//// Arrar de Productos
$products = array(
    ...
    for ($i=0; $i < count($data['productos']); $i++) {
        ...
        $pdf->Cell($w[2],6,$data['productos'][$i]['cantidad'],'1',0,'R');
        $pdf->Cell($w[3],6,SMONEY.number_format(round($data['productos'][$i]['precio'],2), 2, ',', ' '),'1',0,'R');
        $pdf->Cell($w[4],6,SMONEY.number_format(round(intval($data['productos'][$i]['cantidad'])*floatval($data['productos'][$i]['precio']),2), 2, ',', ' '),'1',0,'R');
...
$data2 = array(
    ...
    array("Total", SMONEY.number_format(floatval($data['total']), 2, ',', ' ')),
);
    // Column widths
    $w2 = array(40, 40);
    // Header
